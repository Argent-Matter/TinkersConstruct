...
//  }
  ...
  public BakedModel bake(IGeometryBakingContext owner, ModelBakery baker, Function<Material,TextureAtlasSprite> spriteGetter, ModelState modelTransform, ItemOverrides overrides, ResourceLocation modelLocation) {
    ...
    BakedModel baked = new SimpleBakedModel.Builder(owner.useAmbientOcclusion(), owner.useBlockLight(), true, owner.getTransforms(), overrides).particle(spriteGetter.apply(still)).build();
  ...
  public Collection<Material> getMaterials(IGeometryBakingContext context, Function<ResourceLocation, UnbakedModel> modelGetter, Set<Pair<String, String>> missingTextureErrors) {
    Set<Material> textures = new HashSet<>();
    if (context instanceof BlockGeometryBakingContext block) {
      getTexture(block.owner, "still", textures, missingTextureErrors);
      getTexture(block.owner, "flowing", textures, missingTextureErrors);
      Material overlay = context.getMaterial("overlay");
      if (!isMissing(overlay)) {
        textures.add(overlay);
      }
    }
